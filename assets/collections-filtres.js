...
(function(e) {
  ...
  var obj = {
    ...
    initFiltresEvent: function() {
      if (e(".filtres-js").length > 0) {
        e('.filtres-js:not(.clear-filters) a').unbind().click(function(event) {
          ...
          if(tag.includes("colour")){
             tagOr = "colour";
          }else if(tag.includes("brand")){
             tagOr = "brand";
          }else if(tag.includes("price")){
             tagOr = "price";
          }else if(tag.includes("heels")){
             tagOr = "heels";
          }
                    
          if (tagOr !== ''){
            if (constraintString.includes(tagOr)){
              a1 = true;
              constraintString.split('+').forEach((s)=>{
                var theLength = tagOr.length; 
                if(s.startsWith(tagOr)){
                   c = s.substring(theLength);
                }else if(s.endsWith(tagOr)){
                   c = s.substring(-5);
                  console.log("this ends: " + c);
                }
              })
              
              var n = constraintString.indexOf(c);
              var l = constraintString.length;
              var m = c.length;
              var o = l-m;
              
              var tagToDelete = tagOr+c;
              if(tagOr == "heels"){
                tagToDelete = c;
              }
                
              if(constraintString.indexOf(tagOr) == 0){
                constraintString = constraintString.replace(tagToDelete+"+", "");
              }else if(o == n){
                constraintString = constraintString.replace("+"+tagToDelete, ""); 
              }
          ...
          if(a1){
          	queryParams.constraint = constraintString ? constraintString+'+'+tag : tag;
          }else{
      ...
      function setTextFilters(){  
        var mcsSizeFilterLi = $('.tt-options-swatch li.Size');
        var mcsBrandFilterLi = $('.tt-options-swatch li.Brand');
        var mcsPriceFilterLi = $('.tt-options-swatch li.Price');
        var mcsColourFilterLi = $('.tt-options-swatch li.Colour');
        mcsSizeFilterLi.parent().attr('id', 'mcsSizeFilter');
        mcsBrandFilterLi.parent().attr('id', 'mcsBrandFilter');
        mcsPriceFilterLi.parent().attr('id', 'mcsPriceFilter');
        mcsColourFilterLi.parent().attr('id', 'mcsColourFilter');
        var mcsSizeFilter = $('#mcsSizeFilter li');
        var mcsBrandFilter = $('#mcsBrandFilter li');
        var mcsPriceFilter = $('#mcsPriceFilter li');
        var mcsColourFilter = $('#mcsColourFilter li');
        var i=1;
        var mcsCountSize = mcsSizeFilter.length;
        var mcsCountBrand = mcsBrandFilter.length;
        var mcsCountPrice = mcsPriceFilter.length;
        var mcsCountColour = mcsColourFilter.length;

        for (var n = 0; n < mcsCountSize; ++ n){
          var c1 = 0;
          var mcsSizeSelector = $('#mcsSizeFilter li:nth-child('+i+') a');
          c1 = mcsSizeSelector.text();
          c1 = c1.replace('EU-','');
          mcsSizeSelector.text(c1);
          mcsSizeSelector.addClass("sizeSelector"+i+"");
              i++;
        }
        i=1;
        
        for (var n = 0; n < mcsCountBrand; ++ n){
          var c1 = 0;
          var mcsBrandSelector = $('#mcsBrandFilter li:nth-child('+i+') a');
          c1 = mcsBrandSelector.text();
          c1 = c1.replace('brand-','');
          mcsBrandSelector.text(c1);
              i++;
        }
        i=1;
        
        for (var n = 0; n < mcsCountPrice; ++ n){
          var c1 = 0;
          var mcsPriceSelector = $('#mcsPriceFilter li:nth-child('+i+') a');
          c1 = mcsPriceSelector.text();
          c1 = c1.replace('price-','');
          mcsPriceSelector.text(c1);
              i++;
        }
        i=1;
        
        for (var n = 0; n < mcsCountColour; ++ n){
          var c1 = 0;
          var mcsColourSelector = $('#mcsColourFilter li:nth-child('+i+') a');
          c1 = mcsColourSelector.text();
          c1 = c1.replace('colour-','');
          mcsColourSelector.attr('data-tooltip', c1);
              i++;
        }        
      }
      setTextFilters();  
